...
const storeKeys: StoreKey[] = ["config", "status", "graph", "logs", "taskResult", "testResult"]
 ...
 */
function updateSlice(prevState: Store, key: StoreKey, sliceState: Partial<Store[StoreKey]>): Store {
...
function useApi() {
  ...
  const loadGraph: Loader = (force: boolean = false) => fetchOrReadFromStore("graph", fetchGraph, force)
  const loadStatus: Loader = (force: boolean = false) => fetchOrReadFromStore("status", fetchStatus, force)
...
export const DataProvider: React.FC = ({ children }) => {
  ...
  return <DataContext.Provider value={storeAndActions}>{children}</DataContext.Provider>
}
